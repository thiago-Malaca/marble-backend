...
func (repo *MarbleDbRepository) UpdateCase(ctx context.Context, exec Executor, updateCaseAttributes models.UpdateCaseAttributes) error {
	...
	if updateCaseAttributes.Boost != nil {
...
func (repo *MarbleDbRepository) UnboostCase(ctx context.Context, exec Executor, id string) error {
	return repo.UpdateCase(ctx, exec, models.UpdateCaseAttributes{Id: id, Boost: nil})
}
...
func casesCoreQueryWithRank(pagination models.PaginationAndSorting) squirrel.SelectBuilder {
	orderCondition := fmt.Sprintf("c.boost nulls last, c.%s %s, c.id %s",
...
func orderConditionForCases(p models.PaginationAndSorting) string {
	return fmt.Sprintf("c.boost nulls last, c.%s %s, c.id %s", p.Sorting, p.Order, p.Order)
}
